...
class PlantDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mean_vals = mean_vals.infer_objects().fillna(0).astype(np.float32).values
        means = torch.tensor(mean_vals, dtype=torch.float32)
        return image, means


# === TRAINING FUNCTION ===
def train_one_fold(fold, train_idx, val_idx, full_df):
    print(f"\nFold {fold+1}/{KFOLDS}")
...
def run_kfold_training():
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(df)):
        train_one_fold(fold, train_idx, val_idx, df)
